...
# 目录介绍
- ProTable组件位置 src/components/base/ProTable 
- 使用案例 views/test
- 项目布局/其他代码：开源项目 (https://github.com/Daymychen/art-design-pro)
...
#### 2.3 <span id="demo_img">列表字段设置弹窗</span>
...
const filters = computed<FilterItem[]>(() => [
  ...
  },
  {
    ...
    options: genderEnum,    // 枚举下拉框     （详情参考 src\components\base\ProTable\types\index.ts）
...
const columns = computed<TableColumnItem[]>(() => [
  ...
  },
  {
    label: '性别',
    prop: 'gender',
    options: genderEnum,    // 枚举映射 将后端值映射为枚举对应中文描述
    width: 90,
  },
  {
    label: '操作',
...
| `options` | `EnumItem[]` | `select` / `cascader` 的枚举项。 |
| `getSelectoptions` | `(searchKey) => EnumItem[] \| Promise<EnumItem[]>` | 远程搜索下拉选项。 |
