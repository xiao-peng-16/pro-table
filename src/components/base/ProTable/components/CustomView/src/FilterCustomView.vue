<template>
  <div ref="FilterCustomViewRef" class="filter-custom-view">
    ...
    >
      ...
      <template #column-label="{ row }">
        <!-- label和field展示 -->
        <span style="width: fit-content; cursor: pointer; display: flex; flex-wrap: wrap;">
          <span @click="showRowField = !showRowField" style="margin-right: 5px;">{{ row?.label }}</span>
          <el-tooltip v-if="showRowField" placement="bottom" effect="dark" trigger="click">
            <span style="color: red; white-space: normal">{{ row?.field }}</span>
            <!-- [el-tooltip]插槽 -->
            <template #content>
              <!-- 使用 pre 标签保留换行和空格格式 -->
...
<script setup lang="ts" name="FilterCustomView">
  ...
  import { ElRadioGroup, ElRadio, ElInputNumber, ElButton, ElTooltip, ElScrollbar } from "element-plus"
  ...
  const FilterCustomViewRef = ref<HTMLElement>()
  // table的最大高度 超过则表格内部滚动条
  const tableMaxHeight = computed(() => {
    const viewportRatio = 0.7
    const viewportHeight = window.innerHeight || document.documentElement.clientHeight || 0
    const filterCustomViewEl = FilterCustomViewRef.value
    const tableRootEl = TableRef.value?.getTableRootEl?.()
    if (!viewportHeight || !filterCustomViewEl || !tableRootEl) {
      return undefined
    }
    const filterCustomViewRect = filterCustomViewEl.getBoundingClientRect()
    const tableRect = tableRootEl.getBoundingClientRect()
    return Math.floor(viewportHeight * viewportRatio - (tableRect.top - filterCustomViewRect.top))
  })
