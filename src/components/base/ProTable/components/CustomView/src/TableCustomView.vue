<template>
  <div ref="TableCustomViewRef" class="table-custom-view">
    ...
    >
      <!-- [字段名称]插槽 -->
      <template #column-label="{ row, column, index }">
        <span style="cursor: pointer; white-space: normal" >
          <span @click="showRowField = !showRowField" style="margin-right: 5px">
            <RenderVNode v-if="row?.renderHeader" :vnode="row.renderHeader({ column, index })"/>
            <slot v-else-if="row?.slotHeader" :name="row.slotHeader" :column="column" :index="index"></slot>
            <span v-else>{{ row?.label }}</span>
          </span>
          <el-tooltip v-if="showRowField" placement="bottom" effect="dark" trigger="click">
            <span v-if="row?.render" style="color:dodgerblue;">渲染</span>
            <span v-else-if="row?.slot" style="color:dodgerblue;">插槽</span>
            <span v-else-if="row?.field" style="color: red; white-space: normal">{{ row?.field }}</span>
            <!-- [el-tooltip]插槽 -->
            <template #content>
              <!-- 使用 pre 标签保留换行和空格格式 -->
...
<script setup lang="ts" name="TableCustomView">
  ...
  import { defineComponent, PropType, ref, computed, watch, type VNodeChild } from "vue";
  import { Table } from "@/components/base/ProTable"
  import { VueDraggableProps, TableColumnItem } from "@/components/base/ProTable"
  import { ElSwitch, ElButton, ElTooltip, ElScrollbar } from "element-plus"
  ...
  const TableCustomViewRef = ref<HTMLElement>()
  // table的最大高度 超过则表格内部滚动条
  const tableMaxHeight = computed(() => {
    // 弹窗内表格最大高度基准：取视口高度的 70%
    const viewportRatio = 0.7
    const viewportHeight = window.innerHeight || document.documentElement.clientHeight || 0
    
    const tableCustomViewEl = TableCustomViewRef.value
    const tableRootEl = TableRef.value?.getTableRootEl?.()
    // 首次渲染时 DOM 可能还没挂载完成，此时不设置 max-height，交给 el-table 默认高度处理
    if (!viewportHeight || !tableCustomViewEl || !tableRootEl) {
      return undefined
    }
    const tableCustomViewRect = tableCustomViewEl.getBoundingClientRect()
    const tableRect = tableRootEl.getBoundingClientRect()
    // 扣除 Table 距离 filter-custom-view 顶部的高度：
    // 上方 el-form、p 等内容占用多少空间，表格最大高度就相应减少多少
    return Math.floor(viewportHeight * viewportRatio - (tableRect.top - tableCustomViewRect.top))
  })
