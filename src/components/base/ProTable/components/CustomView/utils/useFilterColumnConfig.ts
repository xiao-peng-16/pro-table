...
// Filter 唯一键策略：field + label。
// 说明：
// 1) 单看 field 在日期范围等场景可能重复（如同 field 不同文案的起止项）；
// 2) 叠加 label 后能稳定区分同 field 的不同筛选项；
// 3) 保持与当前业务诉求一致（不引入 type 参与 key）。
const getFilterKey = (item: FilterItem): string => {
  const fieldKey = Array.isArray(item.field) ? item.field.join('|') : item.field
  const labelKey = item.label ?? ''
  return `${String(fieldKey ?? '')}::${String(labelKey)}`
}

// 构建结构签名：
// - 仅包含结构相关字段（key/field/type）；
...
export const loadFilterOrderConfig = (storageId: string, filters: FilterItem[]): FilterItem[] | null => {
  ...
  try {
    ...
    if (!validateOrderEntries(currentEntries, savedEntries)) {
      ...
      console.log('AAA', savedEntries)
      console.log('BBB', currentEntries)
      clearFilterOrderConfig(storageId)
      return null
    }
