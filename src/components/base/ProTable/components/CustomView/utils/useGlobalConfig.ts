...
// 读取全局配置：
// - 从 localStorage浏览器保存 获取原始数据；
// - 配置字段优先级顺序: 入参配置 > localStorage > 默认配置兜底
// - 归一化后返回；
export const loadGlobalConfig = (highConfig:ProTableGlobalConfig = {}): ProTableGlobalConfig => {
  try {
    // 读取用户浏览器保存自定义配置
    const localConfig = getLocal(getGlobalConfigStorageKey())
    // 配置合并 入参配置 > localStorage
    const newConfig = mergeGlobalConfig(localConfig, highConfig)
    // 兜底操作 空值用默认值补齐
    return normalizeGlobalConfig(newConfig)
  } catch (error) {
    console.log('读取全局配置异常', error); 
    return getDefaultProTalbeGolbalConfig()
  }
}
...
const stripUndefined = <T extends Record<string, unknown>>(obj: unknown): Partial<T> => {
...
const mergeGlobalConfig = (
  oldConfig: Partial<ProTableGlobalConfig> | null | undefined,
  newConfig: Partial<ProTableGlobalConfig> | null | undefined
): ProTableGlobalConfig => {
  const merged: ProTableGlobalConfig = {
    ...stripUndefined<ProTableGlobalConfig>(oldConfig),
    ...stripUndefined<ProTableGlobalConfig>(newConfig),
  }
  return merged
}
