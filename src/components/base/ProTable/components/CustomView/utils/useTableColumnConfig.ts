...
// v2: 列唯一键改为“field 优先”，降低仅文案变更带来的缓存失效概率。
...
// 列唯一键策略：field(字符串) -> label -> slot -> type。
const getColumnKey = (item: TableColumnItem): string => {
  const fieldKey = typeof item.field === 'string' ? item.field : undefined
  return String(fieldKey ?? item.label ?? item.slot ?? item.type ?? '')
}
