...
import { useTableDrag, Item, moveIndex } from '@/components/base/ProTable/utils'
import {
...
export const useTableCustomOperation = (Table_ref: Ref, sourceList_ref: Ref<Item[]>) => {
  ...
  const moveUp = async (item: Item, rowIndex: number, step=1) => {
    return runWithAnimationGuard(async () => {
      ...
      // 等动画结束后再移动数据，避免视觉位置与数据位置不同步。
      // 使用“移动插入”语义：当前项上移到目标位置，被跨越的同级项整体后移；
      // 不能用交换，否则 step > 1 时只会和目标项互换，顺序会不符合拖拽/移动预期。
      if (index - step >= 0) {
        moveIndex(list, index, index - step)
      }
      await nextTick()
  ...
  const moveDown = async (item: Item, rowIndex: number, step=1) => {
    return runWithAnimationGuard(async () => {
      ...
      await waitForTransitionEnd([...currentElList, ...targetElList])
      // 等动画结束后再移动数据，避免视觉位置与数据位置不同步。
      // 使用“移动插入”语义：当前项下移到目标位置，被跨越的同级项整体前移；
      // 不能用交换，否则 step > 1 时只会和目标项互换，顺序会不符合拖拽/移动预期。
