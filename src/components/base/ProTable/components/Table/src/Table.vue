<template>
  <div ref="TableRootRef" class="table-wrapper">
    ...
    <component :is="tableWrapper" v-bind="tableWrapperProps" @update="tableWrapperOnUpdate">
      ...
      >
        <template #default>
          <TableColumn v-for="(item, index) in handleColumns" :key="item.field || item.label || index" :item=item>
              <!-- TableColumn插槽透传 -->
...
<script setup lang="ts">
  ...
  import { getTableColumnSlots, recursionTableColumnList, useTableDrag,  ElTableFns, getElTableFns } from '@/components/base/ProTable/utils'
  ...
  const props = withDefaults(
    ...
    } & /* @vue-ignore */ ElTableProps>(),
    {
      ...
      data: () => [],
      drag: () => null,
      proTableGlobalConfig: () => loadGlobalConfig()
    }
  )
  ...
  // 列表数据：直接使用 props.data
  // 拖拽场景下 emit('update:data', dataList.value) 通知父组件 v-model:data
  ...
  const getValue = (row: any, item: TableColumnItem): { oldValue:any, value:any, tagType:TagTypes } => {
    const oldValue = getFieldValue(row, item.field)
    let value = oldValue
    let tagType = undefined
    if (oldValue !== null && oldValue !== undefined) {
      // 枚举值转换
      if (item.options) {
        let enumItem = item.options.find(e => String(e.value) === String(oldValue))
  ...
  const defaultHandle = (item: TableColumnItem) => {
    if(!item.render && !item.slot && item.field) {
      item.render = (row) => {
  ...
  const typeHandles = {
    ...
    image: (item: TableColumnItem) => {
      ...
      item.render = (row) => {
        const src = getFieldValue(row, item.field)
  ...
  const tableWrapperProps = computed(() => {
    ...
    return {
      ...
      ...(props.drag || {}),
      animation: 150,
      // ghostClass：Sortable 会把该 class 加到“占位行”上。
      // 占位行代表拖拽元素最终将插入的位置，这里用于绘制蓝色插入线。
      ghostClass: 'pro-table-drag-ghost',
      // chosenClass：Sortable 会把该 class 加到“被选中的原始行”上。
      // 鼠标按下并开始拖拽后，用它给原始行增加轻微高亮，帮助识别正在拖哪一行。
      chosenClass: 'pro-table-drag-chosen',
      // dragClass：Sortable 会把该 class 加到“跟随鼠标移动的拖拽镜像”上。
      // 用它调整拖拽中的鼠标样式和镜像行观感，不参与最终数据排序。
      dragClass: 'pro-table-dragging',

      // 滚动条自动拖拽-开始 ********************
      // 明确启用自动滚动。可读性有用，但默认已经是 true，单独加它通常改善不明显。
      scroll: true,
      // 这个最有用。它强制走 JS 自动滚动逻辑，对 Element Plus 这种嵌套滚动容器更稳。
      forceAutoScrollFallback: true,
      // 默认是 30px，也就是鼠标必须很贴近边缘才滚；改成 60-100px 后，拖到表格底部附近更容易触发滚动。
      scrollSensitivity: 50,
      // 滚动快慢 手感参数。默认 10，改 12/15 会滚得更明显，太大容易失控。
      scrollSpeed: 10,
      // 默认是 true，会尝试滚动父级容器；在弹窗/页面里可能导致外层也跟着滚。这里通常希望只滚表格内部，所以建议 false。
      bubbleScroll: false
      // 滚动条自动拖拽-结束 ********************
    }
  })
  ...
  const onDragUpdate = (event: SortableEvent) => {
    ...
    try {
      // 拖拽排序是“移动插入”语义：把旧位置元素移除后插入到新位置，
      // 中间行整体顺移；不能使用 swapItem，否则会变成只和目标行交换位置。
      tableDragHandler.moveItem(dragDataList.value[oldIndex], dragDataList.value[newIndex])
    } catch (e: unknown) {
