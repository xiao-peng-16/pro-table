<template>
  <div ref="ProTableWrapperRef" class="pro-table__wrapper">
    <Filter
      ...
      ref="FilterRef"
      :class="{ 'filter-wrapper--card': proTableGlobalConfig?.proTableCardStyle }"
    ...
    <Teleport to="body" :disabled="!isFullScreen">
      <div
        ...
        class="pro-table__container"
        :class="{
          'pro-table__container--full-screen': isFullScreen,
          'pro-table__container--card': proTableGlobalConfig?.proTableCardStyle && !isFullScreen
        }"
      >
        <!-- Table顶部 -->
...
<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<{
      ...
      getTableData: (params: { [key:string]:any } & PageParam) => PageResponse | any[] | Promise<PageResponse | any[]> // 获取数据方法 返回分页数据 | 直接返回列表
      customId: string,             // 自定义配置的id 区别每个custom
      filterCustom?: boolean        // Filter是否开启搜索条件设置工具
      immediate?: boolean           // 是否立刻请求一次列表数据进行初始化
      isPage?: boolean              // 是否展示分页 isPage(props参数控制) && isShowPage(内部判断) 同时满足时候才渲染分页组件
      config?: ProTableGlobalConfig // [ProTableGlobalConfig全局配置] (字段优先级 props传入配置 > 用户浏览器保存自定义配置 > 默认配置兜底)
  ...
  // 是否展示分页 isPage(props参数控制) && isShowPage(内部判断) 同时满足时候才渲染分页组件
  ...
  const queryData = async () => {
    ...
    try {
      ...
      const pageResponse: PageResponse | any[] = await Promise.resolve(props.getTableData(param))
      ...
      if (Array.isArray(pageResponse)) {
        // 如果接口返回的是列表结构
        dataList.value = pageResponse
        total.value = pageResponse.length
        isShowPage.value = false
      } else {
        // 如果接口返回的分页结构 { 列表, 总条数 }
        dataList.value = pageResponse[PAGE_RESPONSE_DATA_KEY]
  ...
  /**
   * 搜索条件参数变更触发查询事件
   * @param newFilterParam 新的搜索过滤条件参数 （大部分时候 newFilterParam === getQueryParam()， 一个独立Filter控制多个ProTable defineExpose调用时候不一样）
   */
  const onFilterQuery = (newFilterParam: any = undefined) => {
    filterParam.value = {
      ...(newFilterParam ?? {}),
      ...(getQueryParam() ?? {})
    }
  ...
  // --------------------   ProTable全局配置-自定义设置   ----------------------------------------
  // ProTable全局配置
  // 加载全局配置  优先级顺序: 入参配置 > 用户浏览器保存 > 默认配置兜底
  const initGlobalConfig = () => loadGlobalConfig(props.config)
  const proTableGlobalConfig = ref<ProTableGlobalConfig>(initGlobalConfig())
  // 事件 [全局配置] 自定义配置完成 
  const onUpdateCustomGlobal = () =>{
    proTableGlobalConfig.value = initGlobalConfig()
    // 通知[Filter]全局配置已修改
  ...
  const tableMaxHeight = computed<number | string | undefined>(() => {
    ...
    const autoMaxHeightFlag = proTableGlobalConfig.value?.proTableAutoMaxHeight  ?? true
