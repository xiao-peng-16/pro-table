...
export type ShortcutType = 'currentWeek' | 'currentMonth' | 'lastMonth' | 'currentYear' | `lastDay_${number}`
...
export interface FilterItem extends FilterItemType {
    ...
    // onChange事件 newValue：新值  filterForm: filter搜索条件参数form
    onChange?: (newValue: any, filterForm: object) => void,
    // 是否展示该搜索条件
    visible?: boolean
    ...
    getSelectOptions?: (searchKey: string) => EnumItem[] | Promise<EnumItem[]>
    // 搜索条件插槽名称
    slot?: string
    // 扩展
    [key:string]: any
...
export interface TableColumnItem extends TableColumnItemType {
    ...
    labelTooltip?: string
    // 字段名(数据来源) 来源 对应 ElTableColumn.prop 字段名， 在原基础上继续扩展 获取字段值函数
    field?: string | getFieldValueFn
    // 是否展示该字段
    visible?: boolean
...
export type ProTableGlobalConfig = {
  ...
  // [ProTable] Filter/Table 卡片样式
  proTableCardStyle?: boolean
...
export const DEFAULT_PRO_TABLE_GLOBAL_CONFIG: ProTableGlobalConfig = {
  ...
  proTableCardStyle: true,
  proTableSize: 'default',
  filterIsShowAll: true,
  filterShowRow: 1,
  tableIsBorder: true,
  tableIsStripe: true,
  pageDefaultSize: 20,
