...
import { moveIndex, swapIndex } from './listUtil'
...
export const useTableDrag = (sourceList_ref: Ref<Item[]>) => {
  ...
  /**
   * 移动item到目标位置。
   * 该方法用于拖拽排序：被拖拽项会从原位置移除，并插入到目标项所在位置，
   * 中间经过的同级项会整体顺移，而不是只和目标项交换位置。
   * @param item_1 被移动的item
   * @param item_2 目标位置上的item
   * @returns
   */
  const moveItem = (item_1: Item, item_2: Item): void => {

    // 获取nodeId节点所在的数组，拖拽只允许在同级列表内重排。
    let nodeList: Item[] = getSameNodeList(item_1)
    ...
    let nodeListIndex_2 = getNodeIndex(nodeList, item_2)
    if (nodeListIndex_1 === -1 || nodeListIndex_2 === -1) {
      throw new Error("不同层 不允许拖拽排序")
    }

    // 移动插入：符合拖拽排序语义，避免旧逻辑的“交换位置”。
    moveIndex(nodeList, nodeListIndex_1, nodeListIndex_2)
  }
