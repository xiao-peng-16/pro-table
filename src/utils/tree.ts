...
const nodeHandle = (node, config: Partial<TreeHelperConfig> = {}) => {
  node.value = node[config.valueKey]
  node.label = node[config.labelKey]
}

// tree from list
export const listToTree = <T = any>(list: any[], config: Partial<TreeHelperConfig> = {}): T[] => {
  const conf = getConfig(config) as TreeHelperConfig
  const { id, children, parentIdKey, orderFn, orderType, filterFn } = conf
  ...
  for (const node of list) {
    if (filterFn?.(node) ?? true) {
      const parent = nodeMap.get(node[parentIdKey])
      nodeHandle(node, conf)
      if (parent) {
